import { composeSync } from "../ToolsJs.js"; // there wrap.isOptionSelected and wrap.updateSelected should be moved in and
// 
...
export function SelectedPicksPlugin(aspects) {
  ...
  let {
    ...
    addPicked,
    removePicked
    /*,getSelected : getIsOptionSelected, setSelected : setIsOptionSelected*/

  } = configuration; // addPick = (option, key) => updateChoiceSelected(wrapsCollection.get(key), getIsOptionSelected)
  ...
  if (!isOptionPicked) isOptionPicked = option => picks.includes(option);
  if (!addPicked) addPicked = (option, key) => {
    picks.push(option);
  };
  if (!removePicked) removePicked = key => {
    picks.splice(key, 1);
  };
