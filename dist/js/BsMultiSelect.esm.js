      ...
      layout: () => {
        ...
        return {
          buildApi(api) {
            ...
            api.updateOptionsHidden = () => wrapsCollection.forLoop((wrap, key) => updateChoiceHidden(wrap, key, getNextNonHidden, countableChoicesList, getIsOptionHidden, produceChoiceAspect));

            api.updateOptionHidden = key => updateChoiceHidden(wrapsCollection.get(key), key, getNextNonHidden, countableChoicesList, getIsOptionHidden, produceChoiceAspect); // TODO create updateHidden ? 
    ...
    return {
      ...
      layout: () => {
        ...
        function updateDisabled() {
          ...
          let newIsComponentDisabled = (_disabledComponentAsp = disabledComponentAspect.getDisabled()) != null ? _disabledComponentAsp : false;
    ...
    return {
      ...
      layout(newAspects) {
        ...
        for (let i = 0; i < preLayouts.length; i++) {
          ...
          let eventHandler = (_preLayouts$i$value = (_preLayouts$i = preLayouts[i]).value) == null ? void 0 : _preLayouts$i$value.call(_preLayouts$i);
        ...
        for (let j = 0; j < layouts.length; j++) {
          ...
          let eventHandler = (_layouts$j$value = (_layouts$j = layouts[j]).value) == null ? void 0 : _layouts$j$value.call(_layouts$j);
...
function MultiSelectBuilder(environment, plugins, defaultCss) {
  ...
  */
  ...
  let create = (element, options) => {
    ...
    let inlineBuildAspectsList = buildConfiguration == null ? void 0 : buildConfiguration(element, configuration); // next line merges settings.cssPatch and defaults.cssPatch also merge defaults.css and defaults.cssPatch 
