...
var BsMultiSelect = function (window, $, Popper) {
    ...
    _createClass(Plugin, [{
      ...
      value: function hideDropDown() {
        if (this.options.usePopper) {
          ...
          if ($(this.dropDownMenu).hasClass('show')) $(this.dropDownMenu).dropdown('toggle');
        }
      }
    }, {
      key: "showDropDown",
      ...
      value: function appendDropDownItem(itemValue, itemText, isChecked) {
        ...
        var checkBoxId = "dashboardcode-bs-multiselect-".concat(this.input.name.toLowerCase(), "-generated-id-").concat(optionId.toLowerCase());
      ...
      value: function createAndAppendSelectedItem($checkBox, optionId, itemText) {
        ...
        var $selectedItem = $("<li data-option-id=\"".concat(optionId, "\"><span>").concat(itemText, "</span></li>")).css(this.options.selectedItemStyleSys).css(this.options.selectedItemStyle).addClass(this.options.selectedItemClass).insertBefore($(this.filterInputItem));
        var $buttom = $("<button aria-label='Close' tabIndex='-1' type='button'><span aria-hidden='true'>&times;</span></button>").css(this.options.removeSelectedItemButtonStyle).addClass(this.options.removeSelectedItemButtonClass).appendTo($selectedItem);
      ...
      value: function init() {
        ...
        var $container = $("<div/>").addClass(this.options.containerClass).insertAfter($input);
        this.container = $container.get(0);
        var $selectedPanel = $("<ul/>").addClass(this.options.selectedPanelClass).css(this.options.selectedPanelStyleSys).css(this.options.selectedPanelStyle).appendTo($container);
        this.selectedPanel = $selectedPanel.get(0);
        var $filterInputItem = $('<li/>').css(this.options.filterInputItemStyleSys).css(this.options.filterInputItemStyle).addClass(this.options.filterInputItemClass).appendTo($selectedPanel);
        this.filterInputItem = $filterInputItem.get(0);
        var $filterInput = $('<input type="search" autocomplete="off">').css(this.options.filterInputStyleSys).css(this.options.filterInputStyle).addClass(this.options.filterInputClass).appendTo($filterInputItem);
        ...
        if (disabled) {
          ...
          $filterInput.focusout(function (event) {
            if (!_this2.skipFocusout) $(_this2.selectedPanel).removeClass("focus");
          });
  ...
  return Plugin;
}(window, _jquery.default, _popper.default);

var _default = BsMultiSelect;
